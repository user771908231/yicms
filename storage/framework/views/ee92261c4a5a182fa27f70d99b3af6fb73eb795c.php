...
<div class="row">
    <div class="col-sm-12">
        <div class="ibox-title">
            <h5><?php echo e($lists->accessControl->ac_name); ?></h5>
        </div>
        <div class="ibox-content">
            ...
            <form class="form-horizontal m-t-md" action="<?php echo e(route('parksetup.update',$lists->ac_id)); ?>" method="post">
                <?php echo csrf_field(); ?>
            ...
            <div  class="stopcarrule">
                ...
                对外开放功能<select name="is_open" id="is_open">
                    <option value="1" <?php if($lists->accessControl->is_open == 1): ?> selected <?php endif; ?>>开启</option>
                    <option value="0" <?php if($lists->accessControl->is_open == 0): ?> selected <?php endif; ?>>关闭</option>
                </select>
                <div>超时时限：<input type='text' name="over_time" id='over_time' value='<?php echo e($lists->accessControl->rules->overtime); ?>' placeholder='建议输入15 必须为数字'>  分设置时间来规定缴费后挪车时间</div>
                <div>免费分钟数：<input type='text' name="free" id='free' value='<?php echo e($lists->accessControl->rules->free); ?>' placeholder='超出（）分钟，免费停放'> 分设置时间来规定免费停放时间</div>
                <div>车位总数：<input id="allNumber" name='all_number' type='text' value='<?php echo e($lists->accessControl->garage_number_all); ?>' placeholder='请输入车位总数 长度限制6位'>位</div>
                <hr/>
                <div id="setInfo">
                    <h4 style="font-size:18px;color:#3598db;">计费方式（任选其一）</h4>
                    <b>方式一：（按天）</b>
                    <div>单价设置：<input id='unit_price_day' name="unit_price_day" type='text' value='<?php if($lists->accessControl->rules->type == 1): ?> <?php echo e($lists->accessControl->unit_price); ?><?php endif; ?>' placeholder='金额'>元/天</div>
                    <br>
                    <b>方式二：（按小时）</b>
                    <div>单价设置：<input id='unit_price_hour' name='unit_price_hour' type='text' value='<?php if($lists->accessControl->rules->type == 2): ?> <?php echo e($lists->accessControl->unit_price); ?><?php endif; ?>' placeholder='金额'>元/小时</div>
                    <br>
